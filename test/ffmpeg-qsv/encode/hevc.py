...
class seek(HEVC8EncoderTest):
  def init(self, tspec, case, gop, bframes, bitrate, qp, maxrate, profile, rcmode, seek):
    vars(self).update(tspec[case].copy())
    vars(self).update(
      rcmode     = rcmode,
      bframes    = bframes,
  ...
  def test(self, case, gop, bframes, bitrate, qp, maxrate, profile, rcmode, seek):
    self.init(spec, case, gop, bframes, bitrate, qp, maxrate, profile, rcmode, seek)
    self.encode()
